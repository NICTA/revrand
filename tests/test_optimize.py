...
def test_unbounded(make_quadratic):
    ...
    for updater in [SGDUpdater, AdaDelta, AdaGrad, Momentum]:
        res = sgd(qobj, w0, data, eval_obj=True, maxiter=3000,
                  updater=updater())
        assert_opt(*res['x'])
...
def test_sgd_seqdata(make_quadratic):
    ...
    res = sgd(q_seq, w0, data=(x, y), eval_obj=True, maxiter=3000,
              updater=AdaDelta())
    assert_opt(*res['x'])
...
def test_bounded(make_quadratic):
    ...
    res = sgd(qobj, w0, data, bounds=bounds, eval_obj=True, maxiter=3000)
...
def test_structured_params(make_quadratic):
    ...
    res = nsgd(qobj_struc, w0, data, eval_obj=True, maxiter=3000)
    assert_opt(*res.x)
...
def test_log_params(make_quadratic):
    ...
    res = nsgd(qobj, w0, data, eval_obj=True, maxiter=3000,
               bounds=bounds)
    assert_opt(*res.x)
