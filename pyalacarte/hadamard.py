...
sequence ordering requires a re-ordering afterwards incurring a similar overhead
...
# def main():
#         leng = 512
#         M = np.argsort(np.sin(np.arange(leng))).astype(float)
#         M = np.vstack((M,M,M, np.random.permutation(leng)))
#         print('M is:', M)
#         import julia
#         j = julia.Julia()
#         hd = julia.core.JuliaModuleLoader(j).load_module("Hadamard")
#         vX = hd.fwht(M,2)
#         print('Julia:',vX)
#         print('Me:', hadamard(M.copy()))
#         print('Basic:', hadamard_basic(M.copy()))
...
def hadamard(Y, ordering=True):
    """ *Very fast* Hadamard transform for single vector at a time
     Args:
     Y (numpy array): the n*2^k data to be 1d hadamard transformed
     ordering (optional=True): reorder from Walsh to sequence

     Returns:
     (numpy array): hadamard transformed data.
     """
