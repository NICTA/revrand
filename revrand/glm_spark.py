...
def learn(rdd, likelihood, lparams, basis, bparams, reg=1., postcomp=10,
          maxit=1000, tol=1e-7, batchsize=100, rate=0.9,
    ...
    # Broadcast variables for spark workers
    sc = rdd.context
    bcBasis         = sc.broadcast(basis)
    bcLikelihood    = sc.broadcast(likelihood)
    bcPcat          = sc.broadcast(pcat)

    res = sgd_spark(L2, pcat.flatten([m, C, reg, lparams, bparams]),
                    rdd, rate=rate, eta=eta, bounds=bounds, gtol=tol,
                    passes=maxit, batchsize=batchsize, eval_obj=True)
